...
{
    public class MembershipTableManager : IHealthCheckParticipant, ILifecycleParticipant<ISiloLifecycle>, IDisposable
    {
            ...
            GrainTypeManager grainTypeManager)
        {
                ...
                this.snapshot)
            {
                OnPublished = update =>
                {
                    ...
                    try
                    {
                        foreach (Action action in this.updateSubscriptions)
                        {
                            action();
                        }
                    }
                    ...
                    {
                        this.log.LogError("Error while calling an update subscription");
                    }
                }
            };
        ...
        private readonly List<Action> updateSubscriptions = new List<Action>();

        public void SubscribeToUpdate(Action action)
        {
            this.updateSubscriptions.Add(action);
        }

        public string GrainTypeMap => this.grainTypeMap;
        ...
        private void KillMyselfLocally(string reason)
        {
            ...
            if (!this.IsGlobal)
            {
                // Do not exit for global ring failure.
                // If it's a real fatal error, local ring should be able to exit it.
